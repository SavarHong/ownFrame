...
(function (){
    ...
    window.SavarJs = function (params){
    	...
    	for (var param in params) {
            sj.params[param] = params[param];
        }

    	// Touch 事件, 主要为处理滑动的事件和方法
    	sj.touchEvents = {
			start: sj.support.touch ? 'touchstart' : 'mousedown',
			move: sj.support.touch ? 'touchmove' : 'mousemove',
			end: sj.support.touch ? 'touchend' : 'mouseup'
    	};
        /*==================================================================
        ************   操作成功浮层，可传入不同文本 与图标样式  ************
        ...
        sj.showSucceedTips = function (tipsText, tipsIcon) {
            ...
            setTimeout(function () {sj.hideSucceedTips();}, 1000);
        };
        ...
        ************                          倒计时模块                     ************
        ...
        //    var d = Date.UTC(2016, 3, 4, 18, 34); 传入时间
        //    var obj = {
        //         sec: document.getElementById("sec"),  各时间所在元素
        //         mini: document.getElementById("min"),
        //         hour: document.getElementById("hour")
        //    }
        //    SJS.fnTimeCountDown(d, obj);

        sj.fnTimeCountDown = function(d, o){
            var f = {
                zero: function(n){
                    var n = parseInt(n, 10);
                    if(n > 0){
        ...
        ************   拉动页面   ************
        ======================================================*/
        sj.initPullToRefresh = function (pageContainer) {
            ...
            function handleTouchMove(e) {
                ...
                if (!isMoved) {
                    ...
                    if (container[0].scrollHeight === container[0].offsetHeight || sj.device.os !== 'ios') {
                        useTranslate = true;
                    }
                    else {
                        useTranslate = false;
                ...
                if (touchesDiff > 0 && scrollTop <= 0 || scrollTop < 0) {
                    // iOS 8 fix
                    if (sj.device.os === 'ios' && parseInt(sj.device.osVersion.split('.')[0], 10) > 7 && scrollTop === 0 && !wasScrolled) useTranslate = true;
            ...
            function handleTouchEnd(e) {
                ...
                if (refresh) {
                    ...
                    container.trigger('refresh', {
                            ...
                            sj.pullToRefreshDone(container);
                        }
                    });
                }
                else {
            ...
            // 绑定事件
            eventsTarget.on(sj.touchEvents.start, handleTouchStart);
            eventsTarget.on(sj.touchEvents.move, handleTouchMove);
            eventsTarget.on(sj.touchEvents.end, handleTouchEnd);

            if (page.length === 0) return;
            function destroyPullToRefresh() {
                eventsTarget.off(sj.touchEvents.start, handleTouchStart);
                eventsTarget.off(sj.touchEvents.move, handleTouchMove);
                eventsTarget.off(sj.touchEvents.end, handleTouchEnd);
            }
            eventsTarget[0].QDestroyPullToRefresh = destroyPullToRefresh;

        };

        sj.pullToRefreshDone = function (container) {
            container = $(container);
        ...
        sj.pullToRefreshTrigger = function (container) {
            container = $(container);
        ...
        sj.destroyPullToRefresh = function (pageContainer) {
        ...
        ************   无限滚动   ************
        ...
        sj.attachInfiniteScroll = function (infiniteContent) {
            $(infiniteContent).on('scroll', handleInfiniteScroll);
        };
        sj.detachInfiniteScroll = function (infiniteContent) {
            $(infiniteContent).off('scroll', handleInfiniteScroll);
        };

        sj.initInfiniteScroll = function (pageContainer) {
            pageContainer = $(pageContainer);
            var infiniteContent = pageContainer.find('.infinite-scroll');
            if (infiniteContent.length === 0) return;
            sj.attachInfiniteScroll(infiniteContent);
        };
        /* ===============================================================================
        ************   瀑布流（两列简易瀑布流）   ************
        =============================================================================== */
        sj.waterFall = function (data) {
            ...
            w.createHtml = function (imageUrl, appSchemaUrl) {
                var strHtml = '<img class="waterfall-img lazy" src="'+ siteContext.waterfallLoading +'" data-src="' + imageUrl + '" alt="" >';
        ...
        sj.initWaterFallScroll = function (pageContainer) {
            pageContainer = $(pageContainer);
            var waterFallContent = pageContainer.find('.waterfall-scroll');
            if (waterFallContent.length === 0) return;
            sj.attachInfiniteScroll(waterFallContent);
        }
        /* ===============================================================================
        ************   Tabs   ************
        =============================================================================== */
        sj.showTab = function (tab, tabLink) {
            var newTab = $(tab);
        ...
        ************   滑动组件   ************
        ...
        var Slider = function (container, params) {
            ...
            var inverter = isH ? (sj.rtl ? -1 : 1) : 1;

            s.updateSlides = function () {
                s.slides = s.wrapper.children('.' + s.params.slideClass);

                if (s.params.spaceBetween !== 0) {
                    var marginProp = sj.rtl ? 'marginLeft' : 'marginRight';
                    if (isH) {
            ...
            s.attachEvents = function (detach) {
                ...
                s.container[action](sj.touchEvents.start, s.onTouchStart);
                s.container[action](sj.touchEvents.move, s.onTouchMove);
                s.container[action](sj.touchEvents.end, s.onTouchEnd);
        ...
        sj.slider = function (container, params) {
            return new Slider(container, params);
        };
        sj.initSlider = function (pageContainer) {
            ...
            for (var i = 0; i < sliders.length; i++) {
                ...
                var _slider = sj.slider(slider[0], params);
                destroySliderOnRemove(_slider);
            }
        };
        sj.reinitSlider = function (pageContainer) {
            var page = $(pageContainer);
            var sliders = page.find('.slider-init');
            if (sliders.length === 0) return;
            for (var i = 0; i < sliders.length; i++) {
        ...
        *******************               ********************************
        =================================================================*/
        sj.initImagesLazyLoad = function (pageContainer) {
            pageContainer = $(pageContainer);
            ...
            function loadImage(el) {
                ...
                function onLoad() {
                    ...
                    if (sj.params.imagesLazyLoadSequential) {
            ...
            function isElementInViewport (el) {
                ...
                var threshold = sj.params.imagesLazyLoadThreshold || 0;
                return (
        ...
        *****************************  图片预加载  *****************************
        =======================================================================*/
        sj.preLoader = function(res, params) {
            if(!res){
              return ;
            }
            var defaults = {
        ...
        ************   插件来自: https://github.com/ftlabs/fastclick   ************
        ===============================================================================*/
        sj.initFastClicks = function () {
            if (sj.params.activeState) {
            ...
            function findActivableElement(e) {
                var target = $(e.target);
                var parents = target.parents(sj.params.activeStateElements);
            ...
            function targetNeedsFocus(el) {
                ...
                if (tag === 'select') {
                    if (sj.device.os === 'android') return false;
                    else return true;
                }
            ...
            function targetNeedsPrevent(el) {
                ...
                if (el.is('label') || el.parents('label').length > 0) {
                    if (sj.device.os === 'android') {
                        var osv = sj.device.osVersion.split('.');
            ...
            function handleTouchMove(e) {
                ...
                var _isMoved = false;
                var distance = sj.params.fastClicksDistanceThreshold;
                if (distance) {
            ...
            function handleTouchEnd(e) {
                ...
                if (sj.device.os === 'ios' && scrollParent) {
                ...
                var eventType = 'click';
                if (sj.device.os === 'android' && targetElement.nodeName.toLowerCase() === 'select') {
                    eventType = 'mousedown';
                }
        ...
        ************   预绑定一些点击事件，省去JS书写    ************
        ...
        sj.initClickEvents = function () {
            function handleClicks(e) {
                ...
                if (clicked.hasClass('tab-link')) {
                    isTabLink = true;
                    sj.showTab(clicked.attr('data-tab') || clicked.attr('href'), clicked);
                }
            }
        ...
        sj.init = function () {
            ...
            // 初始化点击事件
            if (sj.initFastClicks && sj.params.fastClicks) sj.initFastClicks();
            if (sj.initClickEvents) sj.initClickEvents();
            // 初始化无限滚动
            if (sj.initInfiniteScroll) sj.initInfiniteScroll('.page-wrap');
            // 初始化滑动组件
            if (sj.initSlider) sj.initSlider('.page-wrap');
            // 初始化拉动更新
            if (sj.initPullToRefresh) sj.initPullToRefresh('.page-wrap');
            // 初始化简易瀑布流
            if (sj.initWaterFallScroll) sj.initWaterFallScroll('.page-wrap');
            // 初始化简易瀑布流
            if (sj.initImagesLazyLoad) sj.initImagesLazyLoad('.page-wrap');

        };
        // 执行初始化方法
        if (sj.params.init) sj.init();
    ...
    ************  功能检测   ************
    =====================================*/
    SavarJs.prototype.support = (function () {
        var support = {
    ...
    **********  设备/系统检测  **********
    =====================================*/
    SavarJs.prototype.device = (function () {
        var device = {};
